...
class TestCreateBooking():
    ...
    def test_create_booking(self):
        response = post_request(url_create_booking(),headers=common_headers_json(),auth=None,payload=payload_create_booking(),in_json=False)
        verify_https_status_code(response,200)
        bookingid=response.json()["bookingid"]
        print(bookingid)
        return bookingid

    def test_get_booking(self,test_create_booking):
        bookingid=test_create_booking
        a=API_Constants() # calling function from a class
        print (bookingid)
        print(a.url_patch_put_delete_booking(bookingid))
        response = get_request(a.url_patch_put_delete_booking(bookingid),headers=None,auth=None,in_json=False)
        verify_https_status_code(response,200)
    ...
    def test_update_booking(self,test_create_booking):
        bookingid=test_create_booking
