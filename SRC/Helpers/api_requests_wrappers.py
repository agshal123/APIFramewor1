import json
import requests


def post_request(url, headers,auth,payload,in_json):
    post_response = requests.post(url=url,headers=headers, auth=auth,json=payload)
    if in_json is True:
        return post_response.json()
    return post_response

def get_request(url,headers,auth,in_json):
    get_response=requests.get(url,headers=headers, auth=auth)
    if in_json is True:
        return get_response.json()
    return get_response

def patch_request(url,headers,auth,payload,in_json):
    patch_response = requests.patch(url, headers=headers, auth=auth,json=payload)
    if in_json is True:
        return patch_response.json()
    return  patch_response

def put_request(url,headers,auth,payload,in_json):
    put_response = requests.put(url, headers=headers, auth=auth,json=payload)
    if in_json is True:
        return put_response.json()
    return  put_response

def delete_request(url,headers,auth,in_json):
    delete_response = requests.delete(url, headers=headers, auth=auth)
    if in_json is True:
        return delete_response.json()
    return  delete_response
